...
public class BusConnectionResource {
    ...
    public ResponseEntity saveBusConnection(@Valid @RequestBody BusConnection busConnection) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("https://peaceful-sierra-14544.herokuapp.com/bus-lines/" + busConnection.getId())).build();
    }

    @CrossOrigin(allowedHeaders = "*", allowCredentials = "true")
    @GetMapping("/bus-connections")
    public @ResponseBody ResponseEntity getBusConections() {
    ...
    public ResponseEntity deleteBusStopFromBusLine(@PathVariable("id") String id) {
        BusConnection busConnection = busConnectionService.getBusConnectionById(id);
        if (busConnection != busConnectionService.getBusConnectionById(id)) {
    ...
    public @ResponseBody ResponseEntity getBusLineBuBusLineAndCarrier(@PathVariable("busLineId") String busLineId,@PathVariable("carrierId") String carrierId)  {
